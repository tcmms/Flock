...
export default function LightPillar({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        /* Fixed norm so narrower uPillarWidth does not inflate tanh() input (which was hiding width changes). */
        const float WIDTH_NORM_REF = 1.0;
        col = tanh(col * uGlowAmount / WIDTH_NORM_REF);
        
        col -= fract(sin(dot(gl_FragCoord.xy, vec2(12.9898, 78.233))) * 43758.5453) / 15.0 * uNoiseIntensity;

        vec3 rgb = max(col * uIntensity, 0.0);
        float peak = max(max(rgb.r, rgb.g), rgb.b);
        /* No constant alpha offset — that tinted the full quad gray on white. */
        float alpha = clamp(peak * 2.5, 0.0, 1.0);
        alpha *= smoothstep(0.0, 0.028, peak);
        rgb = clamp(rgb * 1.18, 0.0, 1.0);
  ...
  }, [webGLSupported, quality]) // eslint-disable-line react-hooks/exhaustive-deps -- uniforms: separate effects
